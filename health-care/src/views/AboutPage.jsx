  ...
  return (
    <div className="min-h-screen bg-page">
        ...
        <div className="container mx-auto max-w-4xl">
          <div className="bg-white rounded-2xl shadow-sm border border-[var(--color-border-tertiary)] p-8">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 text-brand-navy font-semibold text-sm hover:underline"
            >
      ...
      <section className="py-4 px-4 pb-12">
        <div className="container mx-auto max-w-4xl">
          <h2 className="text-xl font-semibold text-[var(--color-text-primary)] mb-6">Regulated Product Categories</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {DGDA_CATEGORIES.map((cat) => (
              <div
                key={cat.title}
                className="bg-white rounded-xl border border-[var(--color-border-tertiary)] shadow-sm p-6 flex gap-4"
              >
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${cat.color}`}>
                  {cat.icon}
                </div>
                <div>
                  <h3 className="font-semibold text-[var(--color-text-primary)] mb-1">{cat.title}</h3>
                  <p className="text-[var(--color-text-secondary)] text-sm leading-relaxed">{cat.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[var(--color-border-tertiary)] shadow-sm p-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a
                ...
                className="flex items-center gap-3 bg-[var(--color-background-secondary)] rounded-xl px-5 py-4 hover:bg-[var(--color-background-tertiary)] transition-colors"
              >
                <FaPhone className="text-brand-navy" />
                <div>
                  <p className="text-xs text-[var(--color-text-secondary)]">Phone</p>
                  <p className="text-sm font-semibold text-[var(--color-text-primary)]">+880 1800-MED</p>
                </div>
              </a>
              <a
                ...
                className="flex items-center gap-3 bg-[var(--color-background-secondary)] rounded-xl px-5 py-4 hover:bg-[var(--color-background-tertiary)] transition-colors"
              >
                <FaEnvelope className="text-brand-navy" />
                <div>
                  <p className="text-xs text-[var(--color-text-secondary)]">Email</p>
                  <p className="text-sm font-semibold text-[var(--color-text-primary)]">regulatory@MediportBD.com</p>
                </div>
              </a>
              <div className="flex items-center gap-3 bg-[var(--color-background-secondary)] rounded-xl px-5 py-4">
                <FaMapMarkerAlt className="text-brand-navy" />
                <div>
                  <p className="text-xs text-[var(--color-text-secondary)]">Office</p>
                  <p className="text-sm font-semibold text-[var(--color-text-primary)]">Dhaka, Bangladesh</p>
                </div>
              </div>
            </div>
          </div>
