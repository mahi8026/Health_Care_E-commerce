...
export default function ChartSkeleton() {
  return (
    <div className="bg-white rounded-lg p-5 border-[0.5px] border-[var(--color-border-tertiary)] animate-pulse">
      ...
      <div className="h-4 bg-[var(--color-background-muted)] rounded w-48 mb-4"></div>
      ...
      <div className="flex gap-3 items-end h-48">
        ...
        <div className="flex flex-col justify-between h-full pb-1">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-3 bg-[var(--color-background-muted)] rounded w-8"></div>
          ))}
        </div>
        ...
        <div className="flex-1 flex items-end gap-2 h-full">
          {[60, 85, 45, 90, 70].map((height, i) => (
            <div
              key={i}
              className="flex-1 bg-[var(--color-background-muted)] rounded-t"
              style={{ height: `${height}%` }}
            ></div>
          ))}
        </div>
      ...
      <div className="flex gap-2 mt-2 pl-11">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="flex-1 h-3 bg-[var(--color-background-muted)] rounded"></div>
        ))}
      </div>
    </div>
  );
