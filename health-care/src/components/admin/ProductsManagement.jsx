...
export default function ProductsManagement({ openCreateRef }) {
  ...
  const brandMouseDownRef = useRef(false); // tracks if user is clicking a dropdown item

  // SKU auto-generation state
  const [skuGenerating, setSkuGenerating] = useState(false);
  const [skuAutoGenerated, setSkuAutoGenerated] = useState(false);
  const skuGenRef = useRef(null); // debounce ref

  /**
   * Auto-generate SKU whenever BOTH category and brand are set (create mode only).
   * Runs via useEffect so it's never affected by stale closures.
   * Format: MC-{CAT}-{BRAND}-{SEQUENCE}  e.g. MC-DX-SIE-0042
   */
  useEffect(() => {
  ...
  // Keep generateSku as a manual trigger for the regenerate button only
  ...
  const closeModal = () => {
    ...
    brandMouseDownRef.current = false;
  };
                ...
                <div>
                  ...
                  <div className="relative brand-dropdown-container">
                    <input
                      ...
                      onBlur={() => {
                        // Skip blur logic if user is clicking a dropdown item
                        if (brandMouseDownRef.current) return;
                        setTimeout(() => {
                          // Try to match whatever is typed on blur
                          const match = (manufacturers || []).find(
                            m => m.name.toLowerCase() === brandSearch.toLowerCase()
                          );
                          if (match) {
                            setCreateForm(f => ({ ...f, brand: match._id }));
                            setBrandSearch(match.name);
                          } else if (!createForm.brand) {
                            // No match and nothing selected — clear the text to avoid stale hint
                            setBrandSearch('');
                          }
                          setShowBrandDropdown(false);
                        }, 150);
                        ...
                        {(manufacturers || [])
                          ...
                          .map(manufacturer => (
                            <div
                              ...
                              onMouseDown={() => { brandMouseDownRef.current = true; }}
                              onMouseUp={() => { brandMouseDownRef.current = false; }}
                              onClick={() => {
                  ...
                  {/* Only show hint when user has typed something but hasn't selected yet */}
