...
export default function ProductsManagement({ openCreateRef }) {
  ...
  useEffect(() => {
    ...
    skuGenRef.current = setTimeout(async () => {
      ...
      // Derive codes from the already-loaded categories/manufacturers lists
      const cat = (categories || []).find(c => c._id === createForm.category);
      const brand = (manufacturers || []).find(m => m._id === createForm.brand);
      if (cat && brand) {
        const catCode   = getCategoryCode(cat.name);
        const brandCode = getBrandCode(brand.name);
        // Use last 4 digits of timestamp as sequence — unique enough for a draft
        const seq = String(Date.now()).slice(-4);
        const sku = `MC-${catCode}-${brandCode}-${seq}`;
  ...
  const generateSku = useCallback(async (categoryId, brandId) => {
    ...
    if (cat && brand) {
      ...
      const brandCode = getBrandCode(brand.name);
      const seq = String(Date.now()).slice(-4);
      setCreateForm(f => ({ ...f, sku: `MC-${catCode}-${brandCode}-${seq}` }));
      setSkuAutoGenerated(true);
    }
