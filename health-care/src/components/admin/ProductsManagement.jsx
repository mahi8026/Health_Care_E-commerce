...
export default function ProductsManagement({ openCreateRef }) {
  ...
  // Debug: Test Cloudinary loading on component mount
  useEffect(() => {
    if (typeof window !== 'undefined') {
      console.log('[DEBUG] Testing Cloudinary image loading...');
      const testImg = new Image();
      testImg.onload = () => console.log('[DEBUG] ✅ Test image loaded successfully');
      testImg.onerror = (e) => console.error('[DEBUG] ❌ Test image failed to load:', e);
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      console.log('[API RESPONSE]', {
        ...
        hasData: !!data.data,
        productsCount: Array.isArray(data.data) ? data.data.length : 0,
        firstProduct: data.data?.[0] ? {
          name: data.data[0].name,
          hasImages: !!data.data[0].images,
          imagesLength: data.data[0].images?.length,
          firstImage: data.data[0].images?.[0]
        } : null
      });
                  ...
                  <div className="w-14 h-14 rounded-lg overflow-hidden bg-[var(--color-background-secondary)] flex-shrink-0 border-[0.5px] border-[var(--color-border-tertiary)]">
                    {img ? (
                      ...
                      <img 
                        ...
                        loading="lazy"
                        onLoad={(e) => console.log(`[IMG SUCCESS] ${product.name}:`, e.target.src)}
                        onError={(e) => {
                          console.error(`[IMG FAILED] ${product.name}:`, {
                            src: e.target.src,
                            error: e.type,
                      ...
                      <div className="w-12 h-12 rounded-lg overflow-hidden bg-[var(--color-background-secondary)] flex-shrink-0 border-[0.5px] border-[var(--color-border-tertiary)]">
                        {img ? (
                          ...
                          <img 
                            ...
                            referrerPolicy="no-referrer"
                            loading="lazy"
                            onLoad={(e) => console.log(`[IMG SUCCESS DESKTOP] ${product.name}:`, e.target.src)}
                            onError={(e) => {
                              console.error(`[IMG FAILED DESKTOP] ${product.name}:`, {
                                src: e.target.src,
                                error: e.type,
                                naturalWidth: e.target.naturalWidth,
                                naturalHeight: e.target.naturalHeight,
