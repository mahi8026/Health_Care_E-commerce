...
export default function NewsletterManagement() {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg p-4 border-[0.5px] border-[var(--color-border-tertiary)] mb-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-wrap">
          <input
            ...
            className="flex-1 min-w-0 sm:min-w-[200px] px-3 min-h-[48px] text-[16px] sm:text-[12px] border border-[var(--color-border-secondary)] rounded focus:outline-none focus:border-[#0B2545]"
          />

          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="px-3 min-h-[48px] sm:min-h-[36px] text-[16px] sm:text-[12px] border border-[var(--color-border-secondary)] rounded focus:outline-none focus:border-[#0B2545]"
          >
          ...
          <button
            ...
            className="sm:ml-auto min-h-[48px] sm:min-h-[36px] px-4 py-2 bg-[#0B2545] text-white text-[13px] sm:text-[12px] font-medium rounded hover:bg-[#0d2d52] transition-colors"
          >
            📧 Compose Broadcast
          </button>
        </div>
      ...
      {/* ── Mobile Card View (< md) ─────────────────────────────────────── */}
      <div className="md:hidden bg-white rounded-lg border-[0.5px] border-[var(--color-border-tertiary)] overflow-hidden mb-4">
        {loading ? (
          <div className="p-8 text-center text-[13px] text-[var(--color-text-secondary)]">Loading...</div>
        ) : subscribers.length === 0 ? (
          <div className="p-8 text-center text-[13px] text-[var(--color-text-secondary)]">No subscribers found</div>
        ) : (
              ...
              <div key={sub._id} className="p-4">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <p className="text-[13px] font-semibold text-[var(--color-text-primary)] truncate">{sub.email}</p>
                    {sub.name && <p className="text-[11px] text-[var(--color-text-secondary)] mt-0.5">{sub.name}</p>}
                  </div>
                  <span className={`text-[10px] px-2 py-1 rounded font-medium flex-shrink-0 ${
                    sub.isSubscribed ? 'bg-[#D1FAE5] text-[#065F46]' : 'bg-[#FEE2E2] text-[#991B1B]'
                  }`}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] px-2 py-1 rounded bg-[#E0E7FF] text-[#3730A3] font-medium">{sub.source}</span>
                    <span className="text-[11px] text-[var(--color-text-secondary)]">{new Date(sub.subscribedAt).toLocaleDateString('en-BD')}</span>
                  </div>
                  <button
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border-tertiary)]">
            <button
              ...
              disabled={page === 1}
              className="min-h-[44px] px-4 text-[12px] border border-[var(--color-border-secondary)] rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[var(--color-background-secondary)]"
            >
              Previous
            </button>
      ...
      {showBroadcastPanel && (
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-4 sm:p-6 border-b border-[var(--color-border-tertiary)] flex items-center justify-between sticky top-0 bg-white z-10">
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center rounded-full text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-background-secondary)] text-xl transition-colors"
              >
                ×
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 min-h-[48px] text-[16px] sm:text-[13px] border border-[var(--color-border-secondary)] rounded focus:outline-none focus:border-[#0B2545]"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 min-h-[48px] px-4 py-2 bg-[#0B2545] text-white text-[13px] font-medium rounded hover:bg-[#0d2d52] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {broadcasting ? 'Sending...' : '📧 Send Broadcast'}
                </button>
                <button
                  onClick={() => setShowBroadcastPanel(false)}
                  className="min-h-[48px] px-4 py-2 border border-[var(--color-border-secondary)] text-[13px] rounded hover:bg-[var(--color-background-secondary)] transition-colors"
                >
                  Cancel
                </button>
              </div>
