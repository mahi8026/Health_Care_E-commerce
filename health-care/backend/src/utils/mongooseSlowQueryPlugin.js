    ...
    schema.post(method, function(result) {
      ...
      if (logAll || duration > threshold) {
        ...
        const logData = {
          ...
          update: update && Object.keys(update).length > 0 ? JSON.stringify(update) : undefined,
          options: options && Object.keys(options).length > 0 ? JSON.stringify(options) : undefined,
          duration: `${duration}ms`,
          timestamp: new Date().toISOString(),
          slow: duration > threshold,
